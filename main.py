...
class WidgetsExample(GridLayout):
    ...
    def on_button_click(self):
        ...
        if self.count_enabled:
            self.count += 1
            self.my_text =  str(self.count) #use self otherwise it'll be a local variable and you can't use it.
...
class CanvasExample5(Widget):
    ...
    def update(self, dt):
        ...
        if y + self.ball_size > self.height:
            ...
            self.vy = -self.vy  #to send the ball the other way
        if x + self.ball_size > self.width:
            x = self.width- self.ball_size
            self.vx = -self.vx  #to send the ball the other way
        if y < 0:
            y = 0
            self.vy = -self.vy
        if x < 0:
